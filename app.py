...
def page_reports():
    ...
    all_expenses = sum(e['amount'] for e in st.session_state.expenses)
    all_rent = sum(e['amount'] for e in st.session_state.expenses if e['category'] == 'Rent')
    all_electricity = sum(e['amount'] for e in st.session_state.expenses if e['category'] == 'Electricity')
    all_salary = sum(e['amount'] for e in st.session_state.expenses if e['category'] == 'Salary')
    all_transport = sum(e['amount'] for e in st.session_state.expenses if e['category'] == 'Transport')
    all_internet = sum(e['amount'] for e in st.session_state.expenses if e['category'] == 'Internet')
    all_maintenance = sum(e['amount'] for e in st.session_state.expenses if e['category'] == 'Maintenance')
    all_other = all_expenses - all_rent - all_electricity - all_salary - all_transport - all_internet - all_maintenance
    
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("🏠 Rent", f"₹{all_rent:,}")
    c2.metric("⚡ Electricity", f"₹{all_electricity:,}")
    c3.metric("👥 Salary", f"₹{all_salary:,}")
    c4.metric("🚚 Transport", f"₹{all_transport:,}")
    
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("📶 Internet", f"₹{all_internet:,}")
    c2.metric("🔧 Maintenance", f"₹{all_maintenance:,}")
    c3.metric("📦 Other", f"₹{all_other:,}")
    c4.metric("💰 Total", f"₹{all_expenses:,}")
    ...
    total_stock_value_mrp = sum(m['price'] * m['stock'] for m in st.session_state.inventory)
    total_stock_value_cost = sum(m.get('cost', 0) * m['stock'] for m in st.session_state.inventory)
    potential_profit = total_stock_value_mrp - total_stock_value_cost
    
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Total Medicines", len(st.session_state.inventory))
    c2.metric("Stock Value (MRP)", f"₹{total_stock_value_mrp:,}")
    c3.metric("Stock Value (Cost)", f"₹{total_stock_value_cost:,}")
    c4.metric("Potential Profit", f"₹{potential_profit:,}")
    
    # Category-wise inventory
    st.markdown("#### 📊 Category-wise Inventory Cost")
    cat_data = {}
    for m in st.session_state.inventory:
        cat = m.get('category', 'Other')
        if cat not in cat_data:
            cat_data[cat] = {"count": 0, "cost": 0, "mrp": 0}
        cat_data[cat]["count"] += m['stock']
        cat_data[cat]["cost"] += m.get('cost', 0) * m['stock']
        cat_data[cat]["mrp"] += m['price'] * m['stock']
    
    for cat, data in sorted(cat_data.items(), key=lambda x: x[1]['cost'], reverse=True):
        st.write(f"**{cat}:** {data['count']} units | Cost: ₹{data['cost']:,} | MRP: ₹{data['mrp']:,}")
    ...
    with col1:
        st.markdown("#### 💰 Income")
        st.metric("This Month", f"₹{month_income:,}")
        st.metric("Last 6 Months", f"₹{total_6m_income:,}")
        st.metric("This Year", f"₹{year_income:,}")
    
    with col2:
        st.markdown("#### 💸 Expenses")
        st.metric("This Month", f"₹{month_total_expenses:,}")
        st.metric("Last 6 Months", f"₹{total_6m_expenses:,}")
        st.metric("This Year", f"₹{year_expenses:,}")
    
    with col3:
        st.markdown("#### ✅ Net Profit")
        st.metric("This Month", f"₹{net_profit:,}", delta_color="normal" if net_profit >= 0 else "inverse")
        st.metric("Last 6 Months", f"₹{total_6m_net:,}", delta_color="normal" if total_6m_net >= 0 else "inverse")
        st.metric("This Year", f"₹{year_net:,}", delta_color="normal" if year_net >= 0 else "inverse")
